...
// Health score color bands - Task 9 v2
const HEALTH_SCORE_BANDS = {
  excellent: { min: 80, max: 100, color: '#10B981', name: 'Excellent', icon: '🟢', badge: 'EXCELLENT' },
  good: { min: 60, max: 79, color: '#3B82F6', name: 'Good', icon: '🔵', badge: 'GOOD' },
  fair: { min: 40, max: 59, color: '#F59E0B', name: 'Fair', icon: '🟡', badge: 'FAIR' },
  poor: { min: 0, max: 39, color: '#EF4444', name: 'Poor', icon: '🔴', badge: 'POOR' }
};
...
/**
 * Format health badge for display alongside token symbol
 * @param {number} score - Health score (0-100)
 * @param {boolean} useChalk - Whether to use chalk colors
 * @returns {string} Formatted health badge
 */
function formatHealthBadge(score, useChalk = true) {
  const band = getHealthScoreBand(score);
  const icon = band.icon;
  const badge = band.badge;
...
/**
 * Format token display with health badge
 * @param {string} symbol - Token symbol
 * @param {string} mint - Token mint address
 * @param {number} healthScore - Health score (0-100)
 * @param {boolean} useChalk - Whether to use chalk colors
 * @param {number} maxLength - Maximum length for display
 * @returns {string} Formatted token display with health badge
 */
function formatTokenDisplayWithHealth(symbol, mint, healthScore, useChalk = true, maxLength = 50) {
  const symbolStr = symbol || 'UNKNOWN';
  const mintShort = `${mint.slice(0, 4)}…${mint.slice(-4)}`;
  const healthBadge = healthScore !== null ? ` • ${formatHealthBadge(healthScore, useChalk)}` : '';
  const display = `${symbolStr} (${mintShort})${healthBadge}`;
  
  if (display.length > maxLength) {
    const truncatedSymbol = symbolStr.slice(0, maxLength - 8 - mintShort.length - healthBadge.length);
    return `${truncatedSymbol} (${mintShort})${healthBadge}`;
  }
