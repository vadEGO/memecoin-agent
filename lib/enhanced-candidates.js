...
class EnhancedCandidates {
  ...
  getEnhancedCandidates(limit = 20) {
    try {
      // Enhanced gating: liq ≥ $5k, fresh% ≥ 55%, insider% ≤ 10%, sniper% ≤ 8%
      const candidates = this.db.prepare(`
        ...
        WHERE t.health_score IS NOT NULL
          ...
          AND t.liquidity_usd >= 5000
          AND t.fresh_pct >= 0.55
          AND t.insider_pct <= 0.10
          AND t.sniper_pct <= 0.08
        ORDER BY 
          t.health_score DESC,
          COALESCE((
  ...
  formatCandidatesDisplay(candidates) {
    ...
    candidates.forEach(candidate => {
      const row = 
        ...
        (candidate.diversityScore * 100).toFixed(0) + '%';
    ...
    try {
      const stats = this.db.prepare(`
        SELECT 
          ...
          AVG(fresh_pct * 100) as avg_fresh_pct,
          AVG(sniper_pct * 100) as avg_sniper_pct,
          AVG(insider_pct * 100) as avg_insider_pct,
