...
class EnhancedCandidates {
  ...
  getEnhancedCandidates(limit = 20) {
    try {
      // Business-ready gating: hard gates + risk gates + momentum gates
  ...
  generateExplainability(candidate) {
    ...
    if (candidate.liquidity_usd >= 5000) {
      reasons.push(`Liq $${(candidate.liquidity_usd / 1000).toFixed(1)}k ≥ $5k`);
    }
    if (candidate.holders_count >= 80) {
      reasons.push(`Holders ${candidate.holders_count} ≥ 80`);
    }
